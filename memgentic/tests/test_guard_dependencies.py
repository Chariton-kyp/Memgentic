...
def test_no_fp_on_substring_dependency():
    ...
    df = DiffFile(path="memgentic/pyproject.toml", added_lines={42: '    "langchain-core-extras>=1",'})
    assert check(RULE, [df], lambda p: None) == []


def test_no_fp_in_toml_comment():
    df = DiffFile(path="memgentic/pyproject.toml", added_lines={42: "    # langchain-core is banned here"})
    assert check(RULE, [df], lambda p: None) == []
...
def test_fires_in_package_json():
    rule = GuardRule(id="no-lodash", type="banned_dependency", scope="**",
                     targets=["lodash"], message="no lodash")
    df = DiffFile(path="frontend/package.json", added_lines={5: '    "lodash": "^4.17.21",'})
    assert len(check(rule, [df], lambda p: None)) == 1


def test_requirements_vcs_egg_not_truncated():
    rule = GuardRule(id="no-lc", type="banned_dependency", scope="**",
                     targets=["langchain-core"], message="x")
    df = DiffFile(path="requirements.txt",
                  added_lines={1: "git+https://x.com/repo.git#egg=langchain-core"})
    assert len(check(rule, [df], lambda p: None)) == 1
...
def test_poetry_style_pyproject_falls_back_to_fire():
    ...
    rule = GuardRule(id="no-lc", type="banned_dependency", scope="pyproject.toml",
                     targets=["langchain-core"], message="x")
