#!/usr/bin/env bash
# Memgentic checkpoint hook — Codex CLI Stop event.
# Mirrors the Claude Code checkpoint hook but tags events with
# ``tool=codex`` so the daemon attributes them correctly.
set -u
...
try:
    ...
    sys.stderr.write(f"memgentic codex checkpoint: {exc}\n")
