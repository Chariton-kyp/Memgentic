...
```
docs(architecture): add release-automation plan
```
...
- ❌ `WIP` — use `chore(wip):` if you must, but squash-merge it away
...
Install the commitizen pre-commit hook (see PR 7) and either:
...
blocks invalid titles. Individual commits within the PR are
lenient; squash lets you stage messy WIP commits locally.

### In Claude Code sessions

When you (or I, as Claude) am iterating on code, use draft commits
liberally. Before opening the PR, rewrite the branch so the final
commit message (or PR title) is conventional. `git commit --amend`
and `git rebase -i` are your friends.
